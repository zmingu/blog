...
/* 头像 */
#friends-container .friend-left { margin-right: 15px; flex-shrink: 0; }
#friends-container .friend-avatar {
...
#friends-container .friend-card:hover .friend-avatar {
            ...
            const itemsHtml = group.items.map(item => `
                <a class="friend-card" target="_blank" href="${item.url}" title="${item.description}">
                    <div class="friend-left">
                        <img class="friend-avatar" src="${item.avatar}" alt="${item.name}" referrerpolicy="no-referrer" onerror="this.onerror=null;this.src='/images/fixit.min.svg'">
                    </div>
